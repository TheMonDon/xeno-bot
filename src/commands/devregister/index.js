...
module.exports = {
  ...
  async executeMessage(message, args) {
    ...
    if (child && child.stdout) {
      child.stdout.on('data', d => { try { message.channel.send({ content: `stdout: ${String(d).slice(0, 1800)}`, allowedMentions: { repliedUser: false } }).catch(() => {}); } catch (e) {} });
    }
    if (child && child.stderr) {
      child.stderr.on('data', d => { try { message.channel.send({ content: `stderr: ${String(d).slice(0, 1800)}`, allowedMentions: { repliedUser: false } }).catch(() => {}); } catch (e) {} });
    }
  }
};

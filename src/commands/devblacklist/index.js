...
module.exports = {
  ...
  async executeMessage(message, args) {
    ...
    if (!sub || sub === 'current') {
      if (!message.guild) {
        try { await message.reply({ content: 'No guild context; provide a guild ID.', allowedMentions: { repliedUser: false } }); } catch (e) {}
        return;
      }
      ...
      if (!guildId) { try { await message.reply({ content: 'Provide guild ID to remove from blacklist.', allowedMentions: { repliedUser: false } }); } catch (e) {} return; }
      const ok = await lbModel.remove(guildId);
      try { await message.reply({ content: ok ? `✅ Guild ${guildId} removed from global leaderboard blacklist.` : `❌ Failed to remove guild ${guildId} from blacklist.`, allowedMentions: { repliedUser: false } }); } catch (e) {}
      return;
    } else if (/^\d+$/.test(sub)) {
      guildId = sub;
    } else {
    ...
    if (!guildId) {
      try { await message.reply({ content: 'Usage: xen!devblacklist <guildId|current> OR xen!devblacklist remove <guildId|current>', allowedMentions: { repliedUser: false } }); } catch (e) {}
      return;
    }

    try {
      const ok = await lbModel.add(guildId);
      try { await message.reply({ content: ok ? `✅ Guild ${guildId} added to global leaderboard blacklist.` : `❌ Failed to blacklist guild ${guildId}.`, allowedMentions: { repliedUser: false } }); } catch (e) {}
    } catch (err) {
      try { await message.reply({ content: `❌ Error blacklisting guild: ${err && err.message}`, allowedMentions: { repliedUser: false } }); } catch (e) {}
    }
  }
};
